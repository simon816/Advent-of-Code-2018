...
class Unit(Object):
    ...
    def clone(self):
        new = self.__class__(self.world, self.x, self.y)
        new.attack_power = self.attack_power
        new.hp = self.hp
        return new
...
class World:
    ...
    def tick(self):
        ...
        for unit in sorted(self.get_units(), key=reading_order):
            if not unit.tick() or self.elf_dead:
                return False
        return True
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        x = 0
        for c in line.strip():
            objects.append(to_component(c, None, x, y))
            x += 1
        y += 1
    not_enough = True
    attack_power = 4
    while not_enough:
        ...
        for obj in objects:
            ...
            if isinstance(obj, Elf):
                obj.attack_power = attack_power
            world.add_object(obj.x, obj.y, obj)
        while not world.combat_ended():
            if world.tick():
                rounds += 1
            if world.elf_dead:
                break
        not_enough = world.elf_dead
        attack_power += 1
    hit_points = sum(map(lambda u: u.hp, world.get_units()))
    print(attack_power, rounds, hit_points, rounds * hit_points)
